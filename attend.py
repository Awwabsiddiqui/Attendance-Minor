...
imgs=[] #initialising empty image set for images data in matix form
...
for cl in mylist:  # running through files in the folder
    curimg=cv2.imread(f'{path}/{cl}') # current image name with it's path
    imgs.append(curimg) # add image data matrix to empty set
    classnames.append(os.path.splitext(cl)[0])  #adding raw name of image file/student name to the list of classname
print(classnames)#printing the list of students in class
...
encodeknown=findencodings(imgs) # encodings of the preimages
print("Encoded !") # encoding complete

cap=cv2.VideoCapture(0) # start webcam

while True:
    success , img = cap.read() # success is boolean fro successfull webcam operation or not , image is input of webcam
    ...
    cv2.imshow('Input' , img) # webcam feed shwon
